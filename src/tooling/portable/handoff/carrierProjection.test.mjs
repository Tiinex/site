...
try {
  ...
  assert.deepEqual(built.carrierProjection.routes.map((route) => route.dimension), ['004', '004', '004']);
  assert.deepEqual(built.carrierProjection.routes.map((route) => route.projectedFilename), [
    'tiinex-shared-fixture-004-anchor-to-axiom.handoff-package.zip',
    'tiinex-shared-fixture-004-anchor-to-kodax.handoff-package.zip',
    'tiinex-shared-fixture-004-anchor-to-loom.handoff-package.zip'
  ]);
  ...
  assert.equal(collision.primary.filename, 'tiinex-shared-fixture-004-anchor-to-loom--2.handoff-package.zip');
  assert.equal(collision.selectedRoute.dimension, '004', 'collision suffix must not mutate dimensional lineage');
  ...
  const reversed = manufactureRecipientRelativeHandoffPackage(reversedInput, { packageInput: { builtAt: '2026-08-23T13:00:00.000Z' } });
  ...
  const badInput = await prepareNodeHandoffManufacturingInput({
    workspaceRoot,
    workspaceId: 'shared-fixture',
    handoffPath: routes[0],
  ...
  const bad = manufactureRecipientRelativeHandoffPackage(badInput, { packageInput: { builtAt: '2026-08-23T13:00:00.000Z' } });
  assert.equal(bad.status, 'blocked');
  assert.equal(bad.transportExecutable, false);
  assert.equal(bad.carrierProjection.status, 'blocked');
  ...
  const missingRequired = manufactureRecipientRelativeHandoffPackage(missingRequiredInput, { packageInput: { builtAt: '2026-08-23T13:00:00.000Z' } });
  ...
  assert.equal(missingRequired.transportExecutable, false);
  ...
  const corrected = manufactureRecipientRelativeHandoffPackage(correctedInput, { packageInput: { builtAt: '2026-08-23T13:00:00.000Z' } });
  assert.equal(corrected.status, 'ready', 'shared carrier should become ready once the secondary route required byte is carried');
  assert.equal(corrected.transportExecutable, true);
  ...
  assert.equal(cli.primaryOutput.projectedFilename, 'tiinex-shared-fixture-004-anchor-to-kodax.handoff-package.zip');
