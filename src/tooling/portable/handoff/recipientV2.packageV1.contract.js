...
export function parseHandoffPackageV1(markdown = '') {
  ...
  return Object.freeze({
    ...
    carrierDimension: field(sectionText(body, 'Carrier Continuity'), 'Carrier Dimension'), parentCarrierDimension: field(sectionText(body, 'Carrier Continuity'), 'Parent Carrier Dimension'), carrierCheckpoint: field(sectionText(body, 'Carrier Continuity'), 'Carrier Checkpoint'), majorReason: field(sectionText(body, 'Carrier Continuity'), 'Major Reason'),
...
export function validatePackageFields(value, findings) {
  ...
  if (value?.carrierCheckpoint === 'major') {
    ...
    const missing = ['business', 'docs', 'site'].filter((workspaceId) => !carried.has(workspaceId));
    if (missing.length) findings.push(finding('error', 'portable.handoff-package-v1.major-source-closure-incomplete', 'Major package-v1 carrier requires complete Business, Docs, and Site Workspace snapshot bindings.', { missingWorkspaceIds: missing }));
  }
}
