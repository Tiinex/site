...
function auditRecipientV2(bundle = {}) {
  ...
  const classified = files.filter((file) => {
    const path = String(file.path || '');
    return path === rootPath || path === RECIPIENT_V2_READ_PATH || path === RECIPIENT_V2_TRANSPORT_MANIFEST_PATH || workspacePayloadPaths.has(path) || routePointerPaths.has(path) || participantRolePointerPaths.has(path) || cachePaths.has(path) || /bootstrap\.(?:trace\.md|zip)$/i.test(path);
  }).length;
