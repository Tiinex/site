...
Every manufactured Handoff package also contains `tiinex.package/START.md` and one package-root ordinary `tiinex.pointer.v1` entrypoint per qualified route. A cold consumer may read either orientation surface without executing received package code. START contains one bounded fenced-JSON projection of `workspaces[]`, routes, route-to-workspace binding, and route-selection policy. Each generated Pointer exposes exactly one `Destinations` target to its qualified package-local route and deliberately does not mint Parent continuity. START and Pointers are disposable projections with no semantic authority: `orient-handoff-package <package.zip>` rehydrates the package and independently correlates both against the carrier, closure, manifest, file map, and exact carried workspace bytes; missing, stale, tampered, ambiguous, or mismatched orientation fails closed. Retaining START during this migration preserves prior cold-consumer compatibility; generic Tiinex Viewer traversal of canonical `tiinex.pointer.v1` is not claimed by this portable operation.
...
The human fast path is one package plus the Tooling-returned normal inline routing projection. `--output-dir <dir>` writes the sole primary package using the selected route's workspace-local deterministic projected filename, and the same manufacture result exposes `humanOutput.normalInlineRouting.content` with the exact package-derived orientation that must be rendered adjacent to that package. This normal inline projection is transport-only, disposable, and carries no semantic authority; host presentation may place the exact text in any suitable copyable surface without changing its bytes. `--transport-text` optionally writes the same text to a small disposable sidecar for cross-device/device-loss recovery; that fallback remains `normalEmission: false` and is not part of normal completion. `project-handoff-carrier-output <package.zip> --route <path-or-route-id>` is a read-only regeneration path and exposes the same `humanOutput.primary` plus `humanOutput.normalInlineRouting` contract from package truth. A route id is the unambiguous selector when different workspaces expose the same workspace-relative Handoff path.
...
node tools/tiinex-portable.mjs manufacture-handoff-package ./site --workspace-id site --workspace-roots workspaces.json --workspace-routes routes.json --handoff .topics/handoff/site-to-loom.trace.md --output multi-root-handoff.zip
