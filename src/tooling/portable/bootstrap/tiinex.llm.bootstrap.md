...
Use `describe-cold-start-ingress` to inspect the bounded ingress contract and `qualify-cold-start` to score an explicit observation trace. Preferred-path evidence records native actions before Tiinex takeover, arbitrary reads before orientation/frontier discovery, Tooling calls/bytes/time to orientation, candidate artifacts inspected, and explicit fallback use. A correct eventual answer after broad native archaeology is `recovered-not-preferred`, not a preferred-path pass.
