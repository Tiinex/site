...
export function zoomPlaythingsCamera(camera = {}, factor = 1, anchorX = 0.5, anchorY = 0.5) {
  ...
  const nextZoom = clamp(Number(current.zoom || 1) * Number(factor || 1), 1, 5);
  ...
  return clampCamera({
    ...
    height: nextHeight,
    zoom: nextZoom,
    x: worldAnchorX - nextWidth * ax,
    y: worldAnchorY - nextHeight * ay
  });
}
...
function normalizeCamera(camera = {}) {
  ...
  const zoom = clamp(Number(camera.zoom || worldWidth / Math.max(1, Number(camera.width || worldWidth))), 1, 5);
  const width = Math.min(worldWidth, Math.max(1, Number(camera.width || worldWidth / zoom)));
  const height = Math.min(worldHeight, Math.max(1, Number(camera.height || worldHeight / zoom)));
