...
export function renderCanonicalRelationLocalArtifact(input = {}) {
  ...
  const title = `${scalar(values['Relation Type'])}: ${readableTarget(values['Subject Binding'])} → ${readableTarget(values['Object Binding'])}`;
...
export function qualifyCanonicalRelationLocalArtifact({ markdown = '', schemaMaterials = [], values = {}, parent = null, path = '', continuityMode = '' } = {}) {
  ...
  try {
    ...
    const schemaFindings = relationValidate(parsed);
...
export const relationLocalMaterialization = Object.freeze({
