...
} from './tiinex.interpretation.v1.contract.js';
...
export function qualifyCanonicalInterpretationLocalArtifact({ markdown = '', schemaMaterials = [], values = {}, parent = {}, path = '' } = {}) {
  ...
  try {
    ...
    const schemaFindings = interpretationValidate(parsed);
...
export const interpretationLocalMaterialization = Object.freeze({
  schemaId: CANONICAL_INTERPRETATION_SCHEMA_ID,
  label: 'Interpretation',
  render: renderCanonicalInterpretationLocalArtifact,
  qualify: qualifyCanonicalInterpretationLocalArtifact,
  authoringInputsFromInvocationBindings: ({ parentTarget = '' } = {}) => Object.freeze({ 'Source Target': String(parentTarget || '') }),
