import { isRecordSourceBacked, isRecordLocalSession } from './workspace.authority.js';
export function summarizeWorkspaceMaterial(workspace = {}) {
  ...
  const sourceBackedRecords = records.filter(isRecordSourceBacked);
  const localRecords = records.filter((record) => !isRecordSourceBacked(record) && isRecordLocalSession(record));
