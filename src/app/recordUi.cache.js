...
export function cachedHydratedRecord(record, storage) {
  ...
  const cached = positiveRecordHydration.get(record); if (cached) return cached;
...
export function cachedHydratedWorkspace(workspace, storage) {
  ...
  const records = Array.isArray(workspace.records) ? workspace.records : [], now = Date.now(), cached = workspaceHydration.get(workspace);
  if (cached && cached.records === records && (cached.complete || now - cached.checkedAt < NEGATIVE_RECHECK_MS)) return cached.value;
  const value = hydrateGithubWorkspaceFromSourceCache(workspace, { storage }), hydratedRecords = Array.isArray(value?.records) ? value.records : records;
...
function recordNeedsSourceCacheHydration(record = {}) {
  ...
  const source = record.source || {}, adapter = String(source.adapterId || record.sourceMode || '').toLowerCase();
