...
try {
  ...
  for (const required of ['index.html', 'assets', '.topics/.workspaces/viewer.workspace.md', 'docs/architecture/uc001-workspace-lifecycle.md', 'README.md', 'llms.txt', 'tiinex.build.json', '.nojekyll']) {
  ...
  for (const needle of [
    'react-v158-source-transport-policy', 'UC-001-empty-create-local-workspace', 'No nodes match this view', 'Workspace name is required',
  ...
  if (!String(identity.source || '').includes('v158')) fail('Public build identity should disclose v158 source shell');
  if (failures.length) {
